...
const Container = styled(Flex)`
  ...
  background: linear-gradient(0deg, rgba(39, 38, 44, 0.4), rgba(39, 38, 44, 0.4)),
    linear-gradient(180deg, #d651b3 0%, #492286 100%);
  ${({ theme }) => theme.mediaQueries.md} {
    padding: 0px;
    background: linear-gradient(180deg, #d651b3 0%, #492286 100%);
  }
`
...
const PhishingWarningBanner: React.FC = () => {
  ...
  const warningText = t("please make sure you're visiting https://dex.detrix.io - check the URL carefully.")
  const warningTextAsParts = warningText.split(/(https:\/\/dex.detrix.io)/g)
    ...
    <>
      ...
      {warningTextAsParts.map((text, i) => (
        <Text
          ...
          small
          as="span"
          bold={text === 'https://dex.detrix.io'}
          color={text === 'https://dex.detrix.io' ? '#FFFFFF' : '#BDC2C4'}
        >
          {text}
        </Text>
      ))}

...
const ContributeModal: React.FC<Props> = ({
  ...
  const label = currency === tokens.cake ? t('Max. MByte entry') : t('Max. token entry')
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    poolId === PoolIds.poolBasic
      ? t(
          'For the basic sale, Max MByte entry is capped by minimum between your average MByte balance in the IFO MByte pool, or the pool’s hard cap. To increase the max entry, Stake more MByte into the IFO MByte pool',
        )
      : t(
          'For the unlimited sale, Max MByte entry is capped by your average MByte balance in the IFO MByte pool. To increase the max entry, Stake more MByte into the IFO MByte pool',
        ),
    {},
  )
      ...
      <ModalBody maxWidth="360px">
        <Box p="2px">
          ...
          <Text color="textSubtle" fontSize="12px" mb="24px">
            {t(
              'If you don’t commit enough MByte, you may not receive any IFO tokens at all and will only receive a full refund of your MByte.',
            )}
            <Link
              fontSize="12px"
              display="inline"

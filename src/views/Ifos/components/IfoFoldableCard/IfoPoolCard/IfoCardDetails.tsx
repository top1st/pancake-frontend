...
const MaxTokenEntry = ({ maxToken, ifo, poolId }: { maxToken: number; ifo: Ifo; poolId: PoolIds }) => {
  ...
  const tooltipContent =
    poolId === PoolIds.poolBasic
      ...
      : t(
          'For the unlimited sale, Max MByte entry is capped by your average MByte balance in the IFO MByte pool. To increase the max entry, Stake more MByte into the IFO CAEK pool',
        )

  const { targetRef, tooltip, tooltipVisible } = useTooltip(tooltipContent, { placement: 'bottom-start' })
  const label = isCurrencyCake ? t('Max. MByte entry') : t('Max. token entry')
    ...
    if (status === 'coming_soon') {
      return (
        <>
          ...
          {ifo[poolId].cakeToBurn !== '$0' && <FooterEntry label={t('MByte to burn:')} value={ifo[poolId].cakeToBurn} />}
          <FooterEntry
